    ...
    if colonne==nbcolonnes:
        ...
        if decodeur:
            if feedback(matjeu, matreponse):
                ...
                print("Il faut continuer !")
                # On incrémente la ligne et on remet la colonne à zéro
                ligne+=1
                colonne=0
        else:
            # Le joueur decodeur va pouvoir jouer
            ligne=0
            colonne=0
...
def choisircouleur(matrice,numerocouleur,canvasL,button):
    global ligne
    global nblignes
    global colonne
    global nbcolonnes
    ...
    if decodeur:
        ...
        choisircode(matrice,numerocouleur,canvasL,button)
    return None

def choisircode(matrice,numerocouleur,canvasL,button):
    global ligne
    global colonne
    global dictcouleurs
    global nblignes
...
def feedback(matJ, matR):
    ...
    ListeReponse=matJ[ligne]  # ListeReponse est laaligne en cours
...
def creation_fenetre_principale(canvasL,canvasR):
    ...
    # boutons_couleur[0]=tk.Button(espacecommande,text=dictcouleurs[0],command=lambda:choisircouleur(matjeu,0,canvaslignes,button))
    # boutons_couleur[0].pack(side="left", fill="x", expand=1)
    # boutons_couleur[1]=tk.Button(espacecommande,text=dictcouleurs[1],command=lambda:choisircouleur(matjeu,1,canvaslignes,button))
